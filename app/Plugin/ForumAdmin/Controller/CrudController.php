...
class CrudController extends ForumAdminAppController {
	...
	public function isAuthorized($user = null) {
		...
		list($plugin, $model, $class) = Admin::parseName($this->request->named['model']);
	...
	public function beforeFilter() {
		parent::beforeFilter();
		// Introspect model
		if (isset($this->request->named['model'])) {
			$this->Model = Admin::introspectModel($this->request->named['model']);
			if (!$this->Model) {
				throw new ForbiddenException(__d('admin', 'Invalid Model'));
			}
		}
