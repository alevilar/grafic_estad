...
class SamlComponent extends Component {
	...
	public function __construct(ComponentCollection $collection, array $settings, string $authSource = NULL) {
		...
		if (Configure::read('Saml.AuthSource') != NULL) {
	...
	public function beforeRender(Controller $controller) {
		$controller->set('isAuthenticated', $this->isAuthenticated());
		if ($this->isAuthenticated()) {			
			$attributes = $as->getAttributes();
			CakeSession::write('Auth.User', $attributes);
		}
	}
	 ...
	 */
	public function login($url = array()) {
		try{
			$this->as->login($url);	
		} catch (Exception $e) {
		    echo 'simpleSamlphp Error: ',  $e->getMessage(), "\n";
		}
	}
	 ...
	 */
	public function logout(mixed $url = null) {
		$this->as->logout($url);
	}
