...
class NodeattachmentController extends NodeattachmentAppController {
       ...
       public function admin_nodeIndex($node_id = null) {
              ...
              $attachments = $this->Nodeattachment->find('all', array(
                  'conditions' => array('Nodeattachment.node_id' => $node_id),
                  'order' => array('Nodeattachment.priority ASC', 'Nodeattachment.created ASC')
                      ));
       ...
       public function admin_addStorageFile() {
              ...
              if (empty($storage_path) || empty($this->request['named']['node_id'])) {
                     $this->cakeError('error404');
              }

              $node_id = $this->request['named']['node_id'];

              if (!empty($this->request['named']['file'])) {

                     $File = new File($storage_path . DS . $this->request['named']['file']);
                     ...
                     if ($File->copy($this->uploads_path . DS . $file_name, true)) {
                            ...
                            if ($this->Nodeattachment->save($data)) {
                                   //unlink($storage_path . DS . $this->request['named']['file']);
                                   $notice = array(
       ...
       private function __getMime($file) {
			  ...
			  $exaux = explode('.', $file);
              $ext = strtolower(array_pop($exaux));
       ...
       public function all() {
              ...
              if (isset($this->request['named']['type'])) {
                     $type = $this->Node->Taxonomy->Vocabulary->Type->findByAlias($this->request['named']['type']);
              ...
              if ($this->usePaginationCache) {
                     ...
                     $this->paginate['page'] = isset($this->request['named']['page']) ? $this->request['named']['page'] : 1;
                     ...
                     if (!$nodes) {
                            ...
                            Cache::write($cacheNamePaging, $this->request['paging'], $cacheConfig);
                     } else {
